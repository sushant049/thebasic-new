...
const Step = ({ idx, eyebrow, title, tagline, body, points, image, reverse, id }) => (
  ...
  >
    ...
    <div className="md:col-span-5">
      ...
      <p className="mt-3 font-serif italic text-xl text-[#CF5B2E]">{tagline}</p>
      ...
      <ul className="mt-8 space-y-3">
        {points.map((p) => (
          <li key={p} className="flex items-start gap-3 text-[14px] text-[#1F3A5F]">
            <span className="mt-2 h-px w-6 bg-[#CF5B2E] shrink-0" />
      ...
      <div className="w-full max-w-7xl mx-auto px-6 md:px-12">
        ...
        >
          <p className="font-serif italic text-2xl md:text-3xl text-[#0F2A47] max-w-xl leading-snug">
            &ldquo;Design and operations, owned by one team — accountable to one number: <span className="text-[#CF5B2E]">your yield.</span>&rdquo;
          </p>
          <button
            data-testid="how-cta-consultation"
            onClick={() => document.querySelector("#consultation")?.scrollIntoView({ behavior: "smooth" })}
            className="group inline-flex items-center gap-3 text-[#1F3A5F] hover:text-[#CF5B2E] text-[11px] uppercase tracking-[0.28em] transition-colors"
          >
