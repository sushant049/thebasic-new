        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 mb-20">
          ...
          >
            ...
            <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl font-light text-[#2C2420] leading-[1.05] tracking-tight text-balance">
              ...
              <em className="not-italic text-[#C19A6B]">portfolio company.</em>
            </h2>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-2">
          {reasons.map((r, i) => (
            <motion.div
              ...
              className="group relative py-10 border-t border-[#E5DFD5] flex gap-8 md:gap-12"
            >
              <span className="font-serif text-3xl md:text-4xl font-light text-[#C19A6B] tabular-nums leading-none pt-1 min-w-[3.5rem]">
                {String(i + 1).padStart(2, "0")}
              </span>
              <div className="flex-1">
                <h3 className="font-serif text-2xl md:text-[28px] font-medium text-[#2C2420] leading-tight">
                  {r.title}
                </h3>
                <p className="mt-3 text-[#4A3B32]/85 font-light text-[15px] md:text-base leading-relaxed max-w-md">
                  {r.desc}
                </p>
              </div>
              <div className="absolute bottom-0 left-0 h-px w-0 bg-[#C19A6B] group-hover:w-full transition-all duration-700" />
            </motion.div>
          ))}
        </div>
      </div>
